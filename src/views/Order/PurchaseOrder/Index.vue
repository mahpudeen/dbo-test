<template>
    <v-container fill-height class="main-container">
            ...
            <v-row >
                ...
                <v-col class="d-flex justify-end h70">
                     <v-btn
                        ...
                        class="no-caps mr20"
                    >
                        <span class="text-white">Create Purchase Order</span>
                    </v-btn>
                </v-col>
            </v-row>
        </div>
                        ...
                        <td>
                            <v-menu offset-y>
                                ...
                                <v-list>
                                    ...
                                    <v-list-item :to="'/purchase-order/update/'+props.item.id">
        ...
        >
            <v-card style="border-radius: 15px;">
                ...
                <v-card-title style="margin-top:-50px" class="d-flex justify-center">
                   <span class="fs20">Detail Purchase Order</span>
                </v-card-title>
                <v-card-text>
                    <v-row class="mt-8">
                        ...
                        <v-col cols="12" md="6" class="-mt24">
                            <DetailRow :name="'Supplier'" :value="item.supplier"/>
                        </v-col>
                        <v-col cols="12" md="6" class="-mt24">
                            <DetailRow :name="'Warehouse'" :value="item.warehouse"/>
                        </v-col>
                        <v-col cols="12" md="6" class="-mt24">
                            <DetailRow :name="'Estimated Arrival Date'" :value="item.eta_date"/>
                        </v-col>
                        <v-col cols="12" md="6" class="-mt24">
                            <DetailRow :name="'Total Order'" :value="formatPrice(item.total)"/>
                        </v-col>
        ...
        >
            <v-card>
                <v-card-title>
                    <span class="text-title-modal">Delete Purchase Order</span>
                </v-card-title>
                <v-card-text>
                    <span class="fs16 mt-1">Are you sure want to delete this purchase order?</span>
                </v-card-text>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // This method filters the data based on the supplier and status.
            filterData(data, supplier, status) {
                if (status === 999) {
                    return data.filter(item => {
                        return item.supplier.toLowerCase().includes(supplier.toLowerCase());
                    });
                }
                return data.filter(item => {
                    return item.supplier.toLowerCase().includes(supplier.toLowerCase()) && item.status === status;
                });
            },
            ...
            // This method sends a delete request to the server for a specific purchase order.
            async deletePO(id){
                this.modal_delete = false
                try {
                    const response = await axios.delete('/data/purchaseorder.json', { params: { id: id } });
